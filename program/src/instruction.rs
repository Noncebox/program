...
use crate::ntoken::ntoken::Ntoken;
...
impl SwapInstruction {
    ...
    pub fn pack(&self) -> Vec<u8> {
        ...
        match &*self {
            ...
            Self::Supply(LendAmount{
                amount }) => {
                buf.push(7);
                buf.extend_from_slice(&amount.to_le_bytes());
            }
            Self::Borrow(LendAmount{
                amount }) => {
                buf.push(8);
                buf.extend_from_slice(&amount.to_le_bytes());
            }
            Self::PayBorrow(LendAmount{
                amount }) => {
                buf.push(9);
                buf.extend_from_slice(&amount.to_le_bytes());
            }
            Self::Stake(LendAmount{
                amount }) => {
                buf.push(10);
                buf.extend_from_slice(&amount.to_le_bytes());
            }
            Self::UnStake(LendAmount{
                amount }) => {
                buf.push(11);
                buf.extend_from_slice(&amount.to_le_bytes());
            }
